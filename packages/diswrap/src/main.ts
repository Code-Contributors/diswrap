...
class BotClient {
   ...
   */
  event<EventName extends BotEvents>(
    eventName: EventName,
    handler: (...args: CEvents[keyof CEvents]) => void
  ) {
    /**
     * #### eventMapping
     * This simplifies the CEvents.
     */
    const eventMapping: Record<BotEvents, keyof CEvents> = {
   ...
   */
  loadCog(name: string, cogInstance: any) {
...
export { BotEvents, BotClient, GuessTheNumber, Event, BotIntents, ActivityTypes, PresenceData, ButtonStyle as ButtonStyles, Modal, Button, Embed, SelectMenu, ActionRow, MessageManager, ThreadManager, TextInput, Attachment, Permissions }
